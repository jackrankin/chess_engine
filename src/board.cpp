...
map<ll, string> coordsFor;
map<string, ll> coordsBack;
map<int, char> piece_val;

Board::Board(ll WP, ll WK, ll WQ, ll WB, ll WN, ll WR, ll BP, ll BQ, ll BK, ll BN, ll BB, ll BR, bool turn, ll nevermoved) {
    this->WP = WP;
    this->WR = WR;
    this->WN = WN;
    this->WB = WB;
...
void Board::add_piece(map<char, int>& coord, char board[8][8], char piece, ll bitboard) {
    while (bitboard) {
        string temp = coordsFor[BitManipulation::pop_LSB(&bitboard)];
        int i = (int)(temp[1]) - 49;
        int j = coord[temp[0]];
        board[i][j] = piece;
    }
...
void Board::print_char_board(Board b) {
    map<char, int> coord;
    ...
    Board::add_piece(coord, board, 'K', b.WK);
    Board::add_piece(coord, board, 'R', b.WR);
    Board::add_piece(coord, board, 'Q', b.WQ);
    Board::add_piece(coord, board, 'P', b.WP);
    Board::add_piece(coord, board, 'B', b.WB);
    Board::add_piece(coord, board, 'N', b.WN);

    Board::add_piece(coord, board, 'k', b.BK);
    Board::add_piece(coord, board, 'r', b.BR);
    Board::add_piece(coord, board, 'q', b.BQ);
    Board::add_piece(coord, board, 'p', b.BP);
    Board::add_piece(coord, board, 'b', b.BB);
    Board::add_piece(coord, board, 'n', b.BN);
...
void Board::read_in_char_board(char boardArray[8][8], bool turn) {
    // read in a position
    string WPs = zeroes;
    string WRs = zeroes;
    string WNs = zeroes;
...
ll Board::white_attacks(Board *b) {
    ll attacks = 0;
...
ll Board::black_attacks(Board *b) {
    ll attacks = 0;
...
char Board::captures(Board *b, ll destination) {
    if (b->turn) {
        if (destination & b->BP) {
            return 'p';
...
// THIS NEEDS TO BE REDONE FUCK THIS STUPID SHIT
vector<Move> Board::move_gen1(Board *b) {
...
void Board::order_moves(vector<Move> &moves_list) {
...
void Board::make_move(Board *b, Move move) {
    if (b->turn) {
        if (move.shortcastle) {
            b->WK >>= 2;
...
void Board::undo_move(Board *b, Move move) {
...
vector<int> Board::legal_move(Move move, Board *b) {
    vector<int> res;
...
Move Board::read_in_move(Board *b, string move) {
    Move m;
